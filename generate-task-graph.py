...
f = StringIO('\n'.join(check_output(
    ['./daggen/daggen', '-n', '50', '--fat', '.94,', '--density', '.3',
        '--jump', '4', '--dot'],
...
for v in v_select[:10]:
    soft_bound = min([1.]+[_soft[u] for u in _soft if u in tc.get_in_neighbors(v)])
    m_bound = max([0]+[_WH[u][0] for u in _WH if u in tc.get_in_neighbors(v)])
    K_bound = min([2000]+[_WH[u][1] for u in _WH if u in tc.get_in_neighbors(v)])
...
g.save('complex-task-graph.xml.gz')
